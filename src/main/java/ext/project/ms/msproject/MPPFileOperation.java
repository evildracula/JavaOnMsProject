...
public class MPPFileOperation {
	...
	public Task createTask(String taskName) {
		Task task = null;
		if (tasks != null) {
			task = new Task(Dispatch.invoke(tasks, "Add", Dispatch.Method, new Object[] { taskName }, new int[] { 1 })
					.toDispatch(), msProjApp);
		}
		return task;
	}
	...
	public Task createSummaryTask(String taskName, String nextTaskName, String... childTasks) throws Exception {
		...
		if (tasks != null) {
			Dispatch sumTaskDispatch = Dispatch
					.invoke(tasks, "Add", Dispatch.Method, new Object[] { taskName }, new int[] { 1 }).toDispatch();
			for (int i = 0; i < childTasks.length; i++) {
				Dispatch childTaskDispatch = Dispatch
						.invoke(tasks, "Add", Dispatch.Method, new Object[] { childTasks[i] }, new int[] { 1 })
						.toDispatch();
				if (i == 0) {
					Dispatch.invoke(childTaskDispatch, "OutlineIndent", Dispatch.Method, new Object[] {},
							new int[] { 1 });
				}
			}

			sumTask = new Task(sumTaskDispatch, msProjApp);
			Dispatch newTaskDispatch = Dispatch
					.invoke(tasks, "Add", Dispatch.Method, new Object[] { nextTaskName }, new int[] { 1 }).toDispatch();
	...
	public Task findTaskByName(String taskName) {
		...
		for (int i = 0; i < count; i++) {
			Dispatch curTask = Dispatch.invoke(tasks, "Item", Dispatch.Get, new Object[] { i + 1 }, new int[] { 1 })
					.toDispatch();
	...
	public Resource createResource(String resourceName) {
		...
		Dispatch sumTaskDispatch = Dispatch
				.invoke(resources, "Add", Dispatch.Method, new Object[] { resourceName }, new int[] { 1 }).toDispatch();
	...
	public Resource findResourceByName(String resourceName) {
		...
		for (int i = 0; i < count; i++) {
			Dispatch curResource = Dispatch
					.invoke(resources, "Item", Dispatch.Get, new Object[] { i + 1 }, new int[] { 1 }).toDispatch();
	...
	public Assignment findAssignmentByName(Task task, String resourceName) {
		...
		for (int i = 0; i < count; i++) {
			Dispatch curAssignment = Dispatch
					.invoke(assignmentsDispatch, "Item", Dispatch.Method, new Object[] { i + 1 }, new int[] { 1 })
					.toDispatch();
	...
	public void promoteToSummaryTask(Task task, String... subTasks) {
		...
		if (!isSummary) {
			for (int i = subTasks.length; i > 0; i--) {
				...
				Dispatch subTaskDispatch = Dispatch.invoke(tasks, "Add", Dispatch.Method,
						new Object[] { subTask, task.getTaskID() + 1 }, new int[] { 1 }).getDispatch();
				if (i == subTasks.length) {
					Dispatch.invoke(subTaskDispatch, "OutlineIndent", Dispatch.Method, new Object[] {},
							new int[] { 1 });
				}
			}
		} else {
	...
	public void addCustomField(int tableIdx, String fieldName, int pjField) {
		...
		Dispatch tableDispatch = Dispatch
				.invoke(tablesDispatch, "Item", Dispatch.Get, new Object[] { tableIdx }, new int[] { 1 }).toDispatch();
		System.out.println(Dispatch.get(tableDispatch, "Name").getString());

		Dispatch tableFieldsDispatch = Dispatch
				.invoke(tableDispatch, "TableFields", Dispatch.Get, new Object[] {}, new int[] { 1 }).toDispatch();
		Variant specifiedVar = new Variant();
		specifiedVar.putString(fieldName);
		specifiedVar.putNoParam();
		Dispatch.invoke(tableFieldsDispatch, "Add", Dispatch.Method,
				new Object[] { pjField, specifiedVar, specifiedVar, fieldName }, new int[] { 1 });
	}
